...
export function ProductForm({ productId }: { productId?: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/products/${productId}`)
      ...
      .then((json) => {
        ...
        for (const row of p.inventory || []) inventory[row.size] = String(row.quantity);
        setForm({
          name: p.name,
          sku: p.sku,
          brandId: p.brandId,
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-8">
      ...
      <section className="grid gap-6 rounded-2xl border border-border bg-white p-5 lg:grid-cols-2">
      ...
      <section className="rounded-2xl border border-border bg-white p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <label className="cursor-pointer">
            <span className="inline-flex h-11 items-center rounded-xl border border-border px-4 text-sm font-medium hover:bg-muted">
